...
db_name = System.get_env("DB_NAME", "medigrant_dev")
...
config :medigrant, MedigrantWeb.Endpoint,
  ...
  watchers: [
    esbuild: {Esbuild, :install_and_run, [:medigrant, ~w(--sourcemap=inline --watch)]},
    tailwind: {Tailwind, :install_and_run, [:medigrant, ~w(--watch)]}
  ]
...
config :medigrant, MedigrantWeb.Endpoint,
  ...
  live_reload: [
    ...
    patterns: [
      ...
      ~r"priv/gettext/.*\.po$",
      ~r"lib/medigrant_web/router\.ex$",
      ~r"lib/medigrant_web/(controllers|live|components)/.*\.(ex|heex)$"
    ]
  ]

config :medigrant, dev_routes: true
